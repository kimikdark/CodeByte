...
public class StockFunction {
    ...
    public HttpResponseMessage getStock(
        ...
        @CosmosDBInput(
            ...
            sqlQuery = "SELECT c.id, c.nome, c.quantityInStock FROM c"
        ) String[] stockData,
    ...
    public HttpResponseMessage updateStock(
        ...
        Optional<Integer> novaQuantidadeOptional = request.getBody();
        if (!novaQuantidadeOptional.isPresent()) {
            return request.createResponseBuilder(HttpStatus.BAD_REQUEST)
                .body("Quantidade de stock em falta no corpo do pedido.")
                .build();
        }

        int novaQuantidade = novaQuantidadeOptional.get();
